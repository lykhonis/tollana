...
describe('renderLlmsTxt', () => {
  it('describes Tollana as a journaled runtime', () => {
    ...
    expect(text).toContain('SnapshotTaken')
    expect(text).toContain('host.invoke')
    expect(text).toContain('code.run')
    expect(text).not.toContain('snapshot.exact')
    expect(text).not.toContain('code.isolate')
